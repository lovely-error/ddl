...
fn a_constant_parameter_can_be_the_bound() {
    ...
    let v = compile(concat!(
        "process p (n: u8 = 4, src: buffer in u8, dst: buffer out u8)\n",
        "  loop\n",
...
fn naming_an_array_walks_its_indices() {
    // desc.md:97. The binding is the INDEX rather than a copy of the element:
    ...
    let v = compile(concat!(
        ...
        "  var mem: #[impl(lutram)] [u8; 4] = @zeroed()\n",
        "  loop\n",

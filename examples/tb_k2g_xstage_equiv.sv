...
module tb_k2g_xstage_equiv;
  ...
  task automatic drain();
    while (ref_q.size() > 0 && ddl_q.size() > 0) begin
      automatic logic [45:0] a = ref_q.pop_front();
      automatic logic [45:0] b = ddl_q.pop_front();
      automatic logic [45:0] m = mdl_q.pop_front();
  ...
  task automatic step(logic v, logic r);
    ...
    if (xfer) begin
      ...
      mw_we_value    = ref_packet[45];
      mw_we_tag      = ref_packet[44];
      mw_we_overflow = ref_packet[43];
      mw_we_flag     = ref_packet[42];
      mw_addr        = ref_packet[41:37];
      mw_value       = ref_packet[36:5];
      mw_tag         = ref_packet[4:2];
      mw_overflow    = ref_packet[1];
      mw_flag        = ref_packet[0];
    end
  ...
  initial begin
    ...
    if (compared < 20000 || forwarded < 5000 || stalls < 500 || copies < 5000) begin
      $display("TB_FAIL  vacuous: %0d packets, %0d forwarded, %0d stalls, %0d copies",
               compared, forwarded, stalls, copies);
      $finish;
    end

    if (errors == 0)
      $display("TB_PASS  %0d cycles, %0d comparisons, %0d packets, %0d forwarded, %0d stalls, 0 mismatches",
               cycles, checks, compared, forwarded, stalls);
    else

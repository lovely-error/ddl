...
// that every result leaving is the sum of the two items that went in, PAIRED
// IN ORDER -- a machine that latched the wrong operand would still agree with
// itself.
//
...
module tb_fsm_adder_equiv;
  ...
  task automatic step(logic v, logic [31:0] d, logic r);
    ...
    // Two in, one out. The model holds the first of each pair and pushes the
    // sum when the second arrives.
    if (push) begin
      if (!have_first) begin
        first      = d;
        have_first = 1'b1;
      end else begin
        expect_q.push_back(first + d);
        have_first = 1'b0;
      end
      in_n++;
    end
  ...
  initial begin
    ...
    for (int t = 0; t < 4000; t++)
      step($urandom_range(2) != 0, $urandom, 1'b1);
    ...
    // Half as many results as inputs, so the bar is lower than mul3's.
    if (errors == 0 && out_n > 2000 && stalls > 500)
